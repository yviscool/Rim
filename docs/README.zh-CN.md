...
> 完整功能指南（含启动器命令表、手势表、Vim 键位表）：[功能详解](features.zh-CN.md)。

- **快速启动器：** 搜索并运行索引文件、配置命令及回退动作。灰字补全、历史翻找、`;`/`:`/`|`/`@` 命令前缀一应俱全，详见功能指南。
- **按应用切换的键盘操作：** 可按窗口或进程配置模式、多键序列、计数和动作。TC、资源管理器、记事本均有默认键位，键位表见功能指南。
- **鼠标手势：** 按住配置的鼠标键绘制方向手势；可将方向序列或手势模板映射到动作，并设置应用规则和排除项。右键按住时还可直接用滚轮前后切换任务栏窗口，或点一下左键进入音量模式滚轮调音量，详见[功能详解](features.zh-CN.md)与[滚轮手势专项](gesture-wheel.zh-CN.md)。
