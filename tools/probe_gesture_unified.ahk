...
try {
    ...
    Check("browser V", GestureEngine.ResolveStroke("DR_UR", vPts, "browser.exe", "Browser", "")[1] = "key|browser-v")
    Check("music V", GestureEngine.ResolveStroke("DR_UR", vPts, "music.exe", "Player", "")[1] = "key|music-v")
    Check("global V", GestureEngine.ResolveStroke("DR_UR", vPts, "other.exe", "Other", "")[1] = "key|global-v")
    Check("noglobal", GestureEngine.ResolveStroke("DR_UR", vPts, "locked.exe", "Locked", "")[1] = "")
    Check("direction only", GestureEngine.ResolveStroke("D_R", [{x: 0, y: 0}, {x: 0, y: 20}, {x: 20, y: 20}], "other.exe", "Other", "")[1] = "key|direction-dr")
    ...
    Check("negative scribble unbound", GestureEngine.ResolveStroke("D_R", negPts, "other.exe", "Other", "")[1] = "")
    Check("degenerate single point unbound", GestureEngine.ResolveStroke("D_R", [{x: 5, y: 5}], "other.exe", "Other", "")[1] = "")
    ...
    Loop Parse, negText, "`n", "`r" {
        ...
        if (cPts.Length < 3)
            continue
        negCount++
        if (GestureEngine.ResolveStroke("D_R", cPts, "other.exe", "Other", "")[1] != "")
            negBad++
    ...
    Check("no prefix", GestureRecognizer.Normalize("V") = "V" && GestureRecognizer.Normalize("DR_UR") = "DR_UR")
